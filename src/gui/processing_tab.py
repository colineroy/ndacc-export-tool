...
def classify_files(dirpath):
    ...
    Returns (sharp_files, nogdb_files, mr_files, ignored_files).
    """
    sharp = []
    nogdb = []
    mr = []
...
class ProcessingTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        self._adv_btn = ttk.Button(
            self, text="▸ " + t("advanced"),
            command=self._toggle_advanced, style="Link.TButton",
        )
        self._adv_btn.pack(anchor=tk.W, **pad)

        self._adv_frame = ttk.LabelFrame(self, text="", padding=4)
        self._adv_visible = False

        self.manual_btn = ttk.Button(
            self._adv_frame, text=t("manual_params"),
            command=self._open_manual_params, state=tk.DISABLED,
        )
        self.manual_btn.pack(fill=tk.X)
        self.input_dir_var.trace_add("write", lambda *_: self._check_manual_btn())

        # ── Separator ──
        ttk.Separator(self, orient=tk.HORIZONTAL).pack(fill=tk.X, **pad)

        # ── Run button (big, centered) ──
    ...
    def _toggle_advanced(self):
        ...
        if self._adv_visible:
            ...
            self._adv_btn.configure(text="▸ " + t("advanced"))
        else:
            self._adv_frame.pack(fill=tk.X, padx=16, pady=(0, 4))
            self._adv_btn.configure(text="▾ " + t("advanced"))
    ...
    def _check_manual_btn(self):
        d = self.input_dir_var.get().strip()
        st = tk.NORMAL if d else tk.DISABLED
        self.manual_btn.configure(state=st)

    def _open_manual_params(self):
        from .manual_params_dialog import ManualParamsDialog
        dlg = ManualParamsDialog(self.winfo_toplevel(),
                                 self.input_dir_var.get(), self.i18n)
        self._manual_params = dlg.result or {}
    ...
    def _run(self, input_dir, output_dir):
        ...
        manual_params = getattr(self, "_manual_params", {})

        # 1. Classify files
        sharp_files, nogdb_files, mr_files, ignored = classify_files(input_dir)

        if station == "sdk":
            files = sharp_files + nogdb_files
        else:
            files = mr_files
        ...
        for i, fpath in enumerate(files):
            ...
            try:
                if station == "sdk":
                    if fpath in sharp_files:
                        meta, df = parse_sharp(fpath)
                        if do_dqa:
                            df = _apply_dqa(df, meta)
                    else:
                        meta, df = load_nogdb(
                            fpath, manual_params=manual_params,
                            apply_dqa_flag=do_dqa,
                        )
                else:
        ...
        # 3. Write WOUDC files
        if results:
            self._log(f"[{self.i18n('writing')}] ...")
            written = write_woudc_batch(
                results, output_dir=str(output_dir), version=version,
            )
            self._log(f"  {len(written)} {self.i18n('woudc_written')} → {output_dir}")
        else:
            written = []
        ...
        if no_brewer:
            self._log(
                f"  {len(no_brewer)} {self.i18n('summary_nobrewer')}"
            )
